...
**Updated 2026-08-21, after `feat/human-vote`.** This task used to describe
carving two exceptions — `shape` and `summon` — out of an "observer guard"
that answered `OBSERVER_ONLY` with *"a human has a voice, not a vote"*. That
guard is deleted: there is no longer any `kind`-based check on `grant` or
`deny`, and there never was one on `claim`, `take` or `shape` either — a
human was already unrestricted on all of those before this task starts, and
still is. So there is nothing to carve an exception from, and the task gets
simpler: only `State.birthsAllowed` and the veto itself are genuinely new.
The two tests below that used to assert `claim`/`take`/`shape` were refused
for a human are gone, because that was never true and is not this task's
concern; a test for the actual gate — `summon` refusing an *agent* — replaces
them.
