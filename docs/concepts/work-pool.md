...
(`ownerForPath`, `src/state/work.ts:22-38`). Say one front, FINANCEIRO,
...
is published (`src/state/work.ts:64-87`).
...
(`evidenceFor`, `src/state/work.ts:121-138`; `takeWork`,
`src/state/work.ts:144-184`).
...
(`src/state/work.ts:211-219`), and its earlier nudge state is cleared too, so
returning an item to the pool always starts a clean clock rather than
inheriting whatever staleness it had before (`src/state/work.ts:218`).
...
> (`src/state/work.ts:11-15`, restated at `src/state/work.ts:186-191`)
...
next front from paying for it twice (`src/state/work.ts:139-142`).
...
(`src/state/work.ts:233-251`). The doorbell that rings for one of them is
correspondingly narrow: it fires once an open item has sat for ten minutes
(`DEFAULTS.ORPHAN_POOL_MS`, `src/protocol/types.ts:75-81`) — longer than the
five-minute presence lease on purpose (`DEFAULTS.LEASE_TTL_MS`,
`src/protocol/types.ts:68`), so a front that stopped renewing that long ago
...
(`DEFAULTS.OFFER_TTL_MS`, `src/protocol/types.ts:74`), and the return is
announced by name rather than happening quietly
(`src/state/machine.ts:322-338`).
...
claim gets (`DEFAULTS.ORPHAN_GRACE_MS`, `src/protocol/types.ts:72`;
`src/state/machine.ts:297-320`), so a front that is merely restarting does
...
(`src/cli/main.ts:120-126`). A broken parley never stops the fix the item
