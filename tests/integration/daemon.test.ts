...
describe("a real daemon over a real socket", () => {
  ...
  test("a journal entry that throws is skipped, and the bus still boots", async () => {
    ...
    expect(state.notes).toHaveLength(1);
    // Two branches hardened this path at different layers, and the outer one
    // wins: a `frame: null` never reaches `apply` any more, because the replay
    // guard refuses it on the way in. Either report is the honest one — what
    // must hold is that the loss is SAID, and that the entries after it are
    // still here, which is what abort-on-first-failure would have thrown away.
    const said = written.join("");
    expect(said.includes("skipped 1 journal entry") || said.includes("discarded 1 unreadable journal line")).toBe(true);
  });
  ...
  test("a skipped entry reports the session that went with it, not only itself", async () => {
    ...
    // After the merge this poison is refused by the replay guard rather than
    // thrown by `apply`, so it takes the discard path and this test now walks
    // the same ground as its sibling below. What it still discriminates is the
    // half that matters here: the session-wide loss is counted and said.
    expect(stderr).toMatch(/skipped 1 journal entry|discarded 1 unreadable journal line/);
    ...
    // Pinned against the source rather than this run's stderr: the sentence
    // belongs to the SKIPPED report, and after the merge this poison is
    // discarded instead, so it is no longer reachable from here. It is still a
    // deletion detector, and only that — the measured half above is what
    // discriminates.
    expect(readFileSync(join(import.meta.dir, "..", "..", "src", "daemon", "server.ts"), "utf8"))
      .toContain("counts entries, not the state they would have written");
  });
